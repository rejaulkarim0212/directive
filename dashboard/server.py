...
def save_tasks(tasks):
    ...
    # Trigger refresh (异步，不Blocked，避免僵尸进程)
    def _refresh():
        try:
    ...
    if archive_all_done:
        ...
        for t in tasks:
            if t.get('state') in _TERMINAL_STATES and not t.get('archived'):
                ...
                count += 1
        save_tasks(tasks)
        return {'ok': True, 'message': f'{count} 道Directive已归档', 'count': count}
    task = next((t for t in tasks if t.get('id') == task_id), None)
    if not task:
        return {'ok': False, 'error': f'任务 {task_id} 不存在'}
...
def update_remote_skill(agent_id, skill_name):
    ...
    if not source_json.exists():
        return {'ok': False, 'error': f'技能 {skill_name} 不是远程 skill（None .source.json）'}
...
def handle_create_task(title, org='National Security Council', official='NSC Director', priority='normal', template_id='', params=None, target_dept=''):
    """从看板创建新任务（Presidential Directive模板下旨）。"""
    ...
    if len(title) > 100:
        title = title[:100] + '…'
    # 标题质量校验：防止闲聊被误建为Directive
    if len(title) < _MIN_TITLE_LEN:
        return {'ok': False, 'error': f'标题过短（{len(title)}<{_MIN_TITLE_LEN}字），不像是Directive'}
    if title.lower() in _JUNK_TITLES:
        return {'ok': False, 'error': f'「{title}」不是有效Directive，请输入具体工作指令'}
    ...
    new_task = {
        ...
        'flow_log': [{
            ...
            'from': 'President',
            'to': initial_org,
            'remark': f'Presidential Directive: {title}'
        }],
        'updatedAt': now_iso(),
    }
    if target_dept:
...
def get_agents_status():
    ...
    return {
        ...
        'gateway': {
            'alive': gateway_alive,
            'probe': gateway_probe,
            'status': '🟢 运行中' if gateway_probe else ('🟡 进程在但None响应' if gateway_alive else '🔴 未启动'),
        },
        'agents': agents,
        'checkedAt': now_iso(),
    }
...
def handle_scheduler_retry(task_id, reason=''):
    ...
    sched['lastDispatchTrigger'] = 'chief_of_staff-retry'
    _scheduler_add_flow(task, f'触发重试第{sched["retryCount"]}次：{reason or "超时未推进"}')
    task['updatedAt'] = now_iso()
    save_tasks(tasks)

    dispatch_for_state(task_id, task, state, trigger='chief_of_staff-retry')
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    if not snap_state:
        return {'ok': False, 'error': f'任务 {task_id} None可用回滚快照'}

    old_state = task.get('state', '')
    task['state'] = snap_state
    task['org'] = snapshot.get('org', task.get('org', ''))
    task['now'] = f'↩️ Chief of Staff Office调度自动回滚：{reason or "恢复到上个稳定节点"}'
    task['block'] = 'None'
    sched['retryCount'] = 0
    sched['escalationLevel'] = 0
    sched['stallSince'] = None
    ...
    if snap_state not in _TERMINAL_STATES:
        dispatch_for_state(task_id, task, snap_state, trigger='chief_of_staff-rollback')
...
def handle_scheduler_scan(threshold_sec=180):
    ...
    for task in tasks:
        ...
        if retry_count < max_retry:
            ...
            sched['lastRetryAt'] = now_iso()
            sched['lastDispatchTrigger'] = 'chief_of_staff-scan-retry'
        ...
        if sched.get('autoRollback', True):
            ...
            if snap_state and snap_state != state:
                ...
                task['state'] = snap_state
                task['org'] = snapshot.get('org', task.get('org', ''))
                task['now'] = '↩️ Chief of Staff Office调度自动回滚到稳定节点'
                task['block'] = 'None'
                sched['retryCount'] = 0
                sched['escalationLevel'] = 0
                sched['stallSince'] = None
    ...
    for task_id, state in pending_retries:
        ...
        if retry_task:
            dispatch_for_state(task_id, retry_task, state, trigger='chief_of_staff-scan-retry')
    ...
    for task_id, state in pending_rollbacks:
        ...
        if rollback_task and state not in _TERMINAL_STATES:
            dispatch_for_state(task_id, rollback_task, state, trigger='chief_of_staff-auto-rollback')
...
def handle_repair_flow_order():
    """修复历史任务中首条流转为“President->National Security Council”的错序问题。"""
    tasks = load_tasks()
    fixed = 0
    fixed_ids = []
    ...
    for task in tasks:
        ...
        first = flow_log[0]
        if first.get('from') != 'President' or first.get('to') != 'National Security Council':
            continue

        first['to'] = 'Chief of Staff Office'
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    if agent_id is None and new_state == 'in_progress':
        ...
        agent_id = _ORG_AGENT_MAP.get(org)
    if not agent_id:
        log.info(f'ℹ️ {task_id} 新状态 {new_state} None对应 Agent，跳过自动派发')
        return
    ...
    title = task.get('title', '(None标题)')
    target_dept = task.get('targetDept', '')

    # 根据 agent_id 构造针对性消息
    _msgs = {
        'chief_of_staff': (
            f'📜 PresidentDirective需要你处理\n'
            f'任务ID: {task_id}\n'
            f'Directive: {title}\n'
            f'⚠️ 看板已有此任务，请勿重复创建。直接用 kanban_update.py 更新状态。\n'
            f'请立即转交National Security Council起草执行方案。'
        ),
        'nsc': (
            f'📜 Directive已到National Security Council，请起草方案\n'
            f'任务ID: {task_id}\n'
            f'Directive: {title}\n'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == '/api/task-action':
            ...
            reason = body.get('reason', '').strip() or f'President从看板{action}'
        ...
        if p == '/api/create-task':
            ...
            org = body.get('org', 'National Security Council').strip()
            official = body.get('official', 'NSC Director').strip()

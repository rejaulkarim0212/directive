...
> **Directive**：审查一段 FastAPI 代码的安全性，输出问题清单和修复建议
...
| # | 子任务 | 派发部门 | 说明 |
|---|--------|----------|------|
| 1 | 代码安全扫描 | ⚔️ Department of Defense | 逐文件审查，标记安全问题 |
| 2 | 合规与最佳实践检查 | ⚖️ Department of Justice | 对照 OWASP Top 10 检查合规性 |
| 3 | 修复方案输出 | ⚔️ Department of Defense | 针对每个问题给出修复代码 |
...
**White House Counsel批语：** *"方案合理。Department of Defense负责工程侧审查，Department of Justice负责合规侧审查，分工明确。注意Department of Justice需出具 OWASP 逐项对照表，不能只写'基本合规'。准奏。"*

---

## ⚔️ 各部执行结果

### Department of Defense · 代码安全扫描（耗时 5 分钟）
...
### Department of Justice · OWASP 合规检查（耗时 4 分钟）
...
| A10 | SSRF | ✅ 合规 | None外部请求 |

**Department of Justice结论：** 6/10 项需要处理，其中 2 项不合规（A02、A03），需立即修复。

### Department of Defense · 修复代码（耗时 3 分钟）
...
**Briefing ID**：`JJC-20260221-007`
**总耗时**：14 分钟（规划 1 + 审议 0.5 + 执行 12.5）
**总 Token**：8,200

### 执行摘要

对 320 行 FastAPI 代码Done安全审查。发现 6 个安全问题（2 严重 / 2 高危 / 2 中危），OWASP Top 10 中有 2 项不合规。已提供全部 6 个问题的修复代码。

White House Counsel首轮即准奏，Department of Defense和Department of Justice分工明确：Department of Defense负责代码层面扫描和修复，Department of Justice负责合规框架对照。

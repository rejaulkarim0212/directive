...
def test_dirty_title_cleaned():
    cmd_create('JJC-TEST-E2E-01',
        ...
        'Zhongshu', 'National Security Council', 'NSC Director',
...
def test_pure_path_rejected():
    cmd_create('JJC-TEST-E2E-02', '/Users/bingsen/clawd/openclaw-sansheng-liubu/', 'Zhongshu', 'National Security Council', 'NSC Director')
    assert _get_task('JJC-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('JJC-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Zhongshu', 'National Security Council', 'NSC Director', 'Chief of Staff Office整理Directive')
...
def test_flow_remark_cleaned():
    cmd_create('JJC-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Zhongshu', 'National Security Council', 'NSC Director')
    cmd_flow('JJC-TEST-E2E-04', 'Chief of Staff Office', 'National Security Council', 'Directive传达：审查/Users/bingsen/clawd/xxx项目 Conversation blah')
...
def test_short_title_rejected():
    cmd_create('JJC-TEST-E2E-05', '好的', 'Zhongshu', 'National Security Council', 'NSC Director')
    assert _get_task('JJC-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 传旨前缀剥离
def test_prefix_stripped():
    cmd_create('JJC-TEST-E2E-06', '传旨：帮我写技术博客文章关于智能体架构', 'Zhongshu', 'National Security Council', 'NSC Director')
...
def test_state_update():
    cmd_create('JJC-TEST-E2E-07', '测试状态更新与组织联动功能', 'Zhongshu', 'National Security Council', 'NSC Director')
    cmd_state('JJC-TEST-E2E-07', 'Menxia', '方案提交White House Counsel审议')
    t = _get_task('JJC-TEST-E2E-07')
    assert t is not None
    assert t['state'] == 'Menxia', f"state应为Menxia: {t['state']}"
    assert t['org'] == 'White House Counsel', f"org应为White House Counsel: {t['org']}"


# ── TEST 8: done Done
def test_done():
    cmd_create('JJC-TEST-E2E-08', '测试任务Done状态标记功能', 'Zhongshu', 'National Security Council', 'NSC Director')
    cmd_done('JJC-TEST-E2E-08', '/tmp/output.md', '任务已Done')
...
def test_done_not_overwritable():
    cmd_create('JJC-TEST-E2E-09', '测试已Done任务不可覆盖保护', 'Zhongshu', 'National Security Council', 'NSC Director')
    cmd_done('JJC-TEST-E2E-09', '/tmp/output.md', '任务已Done')
    cmd_create('JJC-TEST-E2E-09', '试图覆盖已Done的任务标题', 'Zhongshu', 'National Security Council', 'NSC Director')

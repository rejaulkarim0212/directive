...
OFFICIALS = [
    ...
    {'id':'wh_counsel', 'label':'White House Counsel', 'role':'White House Counsel', 'emoji':'🔍', 'rank':'EOP'},
    {'id':'omb', 'label':'Office of Management and Budget', 'role':'OMB Director', 'emoji':'📊', 'rank':'EOP'},
    {'id':'cabinet_sec', 'label':'Cabinet Secretariat', 'role':'Cabinet Secretary', 'emoji':'📮', 'rank':'EOP'},
    {'id':'state', 'label':'Department of State', 'role':'Secretary of State', 'emoji':'📝', 'rank':'Cabinet'},
    {'id':'treasury', 'label':'Department of the Treasury', 'role':'Secretary of the Treasury', 'emoji':'💰', 'rank':'Cabinet'},
    {'id':'defense', 'label':'Department of Defense', 'role':'Secretary of Defense', 'emoji':'⚔️', 'rank':'Cabinet'},
    {'id':'justice', 'label':'Department of Justice', 'role':'Attorney General', 'emoji':'⚖️', 'rank':'Cabinet'},
    {'id':'commerce', 'label':'Department of Commerce', 'role':'Secretary of Commerce', 'emoji':'🔧', 'rank':'Cabinet'},
    {'id':'opm', 'label':'Office of Personnel Management', 'role':'Director of OPM', 'emoji':'👔', 'rank':'Agency'},
    {'id':'press_sec', 'label':'Press Office', 'role':'Press Secretary', 'emoji':'📰', 'rank':'EOP'},
]
...
def get_model(agent_id):
    ...
    # 兼容历史：Chief of Staff Office曾使用 main 作为运行时 id
...
def scan_agent(agent_id):
    ...
    if not sj.exists() and agent_id == 'chief_of_staff':
...
def get_task_stats(org_label, tasks):
    ...
    return {'tasks_done':len(done),'tasks_active':len(active),
            'flow_participations':fl,'participated_orders':participated}
    ...
    for off in OFFICIALS:
        ...
        result.append({
            **off,
            ...
            'participated_orders': ts['participated_orders'],

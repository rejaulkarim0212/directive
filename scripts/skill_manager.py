#!/usr/bin/env python3
"""
U.S. Cabinet Branch · Skill 管理工具
支持从本地或远程 URL 添加、更新、查看和移除 skills

Usage:
  python3 scripts/skill_manager.py add-remote --agent nsc --name code_review \\
    --source https://raw.githubusercontent.com/org/skills/main/code_review/SKILL.md \\
    --description "代码审查"
  
  python3 scripts/skill_manager.py list-remote
  
  python3 scripts/skill_manager.py update-remote --agent nsc --name code_review
  
  python3 scripts/skill_manager.py remove-remote --agent nsc --name code_review
  
  python3 scripts/skill_manager.py import-official-hub --agents nsc,wh_counsel,cabinet_sec
"""
import sys
import json
import pathlib
...
SKILL_AGENT_MAPPING = {
    'code_review': ('defense', 'justice', 'wh_counsel', 'omb'),
    'api_design': ('defense', 'commerce', 'wh_counsel'),
    'security_audit': ('justice', 'wh_counsel', 'omb'),
    'data_analysis': ('treasury', 'wh_counsel', 'omb'),
    'doc_generation': ('state', 'wh_counsel'),
    'test_framework': ('commerce', 'justice', 'wh_counsel'),
}
...
def import_official_hub(agent_ids: list) -> bool:
    ...
    for skill_name, url in OFFICIAL_SKILLS_HUB.items():
        ...
        if not agent_ids:
            target_agents = SKILL_AGENT_MAPPING.get(skill_name, ['wh_counsel'])
    ...
    print(f'\n📊 导入Done：{success}/{total} 个 skills 成功')
    if failed:
        print(f'\n❌ 失败列表:')
        for f in failed:
            print(f'   - {f}')
...
def main():
    parser = argparse.ArgumentParser(description='U.S. Cabinet Branch Skill 管理工具', 

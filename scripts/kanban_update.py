...
_AGENT_LABELS = {
    'main': 'Chief of Staff', 'chief_of_staff': 'Chief of Staff',
    'nsc': 'NSC', 'senate': 'Senate', 'omb': 'OMB', 'supreme_court': 'Supreme Court',
    'state_dept': 'Department of State', 'treasury': 'Department of the Treasury', 'dod': 'Department of Defense',
    'doj': 'Department of Justice', 'commerce': 'Department of Commerce',
}
...
def _sanitize_text(raw, max_len=80):
    """清洗文本：剥离文件路径、URL、Conversation 元数据、Executive Order前缀、截断过长内容。"""
    t = (raw or '').strip()
    ...
    # 5) 清理常见前缀: "Executive Order:" "Executive Order:" "directive:" 等
    t = re.sub(r'^(Executive Order|Executive Order|directive|presidential\s*directive)([（(][^)）]*[)）])?[：:\uff1a]\s*', '', t, flags=re.IGNORECASE)
    # 6) 剥离系统元数据关键词
...
def _is_valid_task_title(title):
    """校验标题是否足够作为一个Directive任务。"""
    t = (title or '').strip()
    if len(t) < _MIN_TITLE_LEN:
        return False, f'标题过短（{len(t)}<{_MIN_TITLE_LEN}字），疑似非Directive'
    if t.lower() in _JUNK_TITLES:
        return False, f'标题 "{t}" 不是有效Directive'
    # 纯标点或问号
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    clean_remark = _sanitize_remark(remark) if remark else f"Presidential Directive: {title}"
    def modifier(tasks):
        ...
        tasks.insert(0, {
            ...
            "now": clean_remark[:60] if remark else f"Directive 已创建，等待 {actual_org} 接手",
            "eta": "-", "block": "None", "output": "", "ac": "",
            "flow_log": [{"at": now_iso(), "from": "President", "to": actual_org, "remark": clean_remark}],
            "updatedAt": now_iso()
        })
        return tasks

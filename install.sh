#!/bin/bash
# ══════════════════════════════════════════════════════════════
# Executive Order · OpenClaw Multi-Agent System 一键安装脚本
# ══════════════════════════════════════════════════════════════
set -e
...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  ⚖️  Executive Order · OpenClaw         ║${NC}"
  echo -e "${BLUE}║       安装向导                            ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
check_deps() {
  ...
  if [ ! -f "$OC_CFG" ]; then
    error "未找到 openclaw.json。请先运行 openclaw Done初始化。"
    exit 1
  fi
...
create_workspaces() {
  ...
  AGENTS=(chief_of_staff nsc senate omb supreme_court treasury state_dept dod doj commerce)
...
1. 接到任务先回复"已接旨"。
2. 输出必须包含：任务ID、结果、证据/文件路径、Blocked项。
3. 需要协作时，回复Cabinet Secretariat请求转派，不跨部直连。
...
# ── Step 2: 注册 Agents ─────────────────────────────────────
register_agents() {
  info "注册 Executive Order Agents..."

  # 备份配置
  cp "$OC_CFG" "$OC_CFG.bak.executive-$(date +%Y%m%d-%H%M%S)"
...
AGENTS = [
  {"id": "chief_of_staff", "subagents": {"allowAgents": ["nsc"]}},
  {"id": "nsc", "subagents": {"allowAgents": ["chief_of_staff", "senate", "supreme_court"]}},
  {"id": "senate", "subagents": {"allowAgents": ["nsc", "omb", "supreme_court"]}},
  {"id": "omb", "subagents": {"allowAgents": ["nsc", "senate", "treasury", "state_dept", "dod", "doj", "commerce", "supreme_court"]}},
  {"id": "supreme_court", "subagents": {"allowAgents": ["nsc", "senate", "omb", "treasury", "state_dept", "dod", "doj", "commerce"]}},
  {"id": "treasury", "subagents": {"allowAgents": ["omb", "supreme_court"]}},
  {"id": "state_dept", "subagents": {"allowAgents": ["omb", "supreme_court"]}},
  {"id": "dod", "subagents": {"allowAgents": ["omb", "supreme_court"]}},
  {"id": "doj", "subagents": {"allowAgents": ["omb", "supreme_court"]}},
  {"id": "commerce", "subagents": {"allowAgents": ["omb", "supreme_court"]}},
]
...
tasks = [
    {
        "id": "JJC-DEMO-001",
        "title": "🎉 系统初始化Done",
        "official": "Secretary of Commerce",
        "org": "Department of Commerce",
        "state": "Done",
        "now": "U.S. Cabinet Branch系统已就绪",
        "eta": "-",
        "block": "None",
        "output": "",
        "ac": "系统正常运行",
        "flow_log": [
            {"at": "2024-01-01T00:00:00Z", "from": "President", "to": "National Security Council", "remark": "发布 Presidential Directive：初始化 U.S. Cabinet Branch 系统"},
            {"at": "2024-01-01T00:01:00Z", "from": "National Security Council", "to": "White House Counsel", "remark": "规划方案提交审核"},
            {"at": "2024-01-01T00:02:00Z", "from": "White House Counsel", "to": "Office of Management and Budget", "remark": "法律审议通过，转预算审议"},
            {"at": "2024-01-01T00:03:00Z", "from": "Office of Management and Budget", "to": "Cabinet Secretariat", "remark": "✅ 预算与资源审议通过"},
            {"at": "2024-01-01T00:04:00Z", "from": "Cabinet Secretariat", "to": "Department of Commerce", "remark": "派发：系统初始化"},
            {"at": "2024-01-01T00:05:00Z", "from": "Department of Commerce", "to": "Cabinet Secretariat", "remark": "✅ Done"},
        ]
    }
]
...
build_frontend() {
  ...
  if ! command -v node &>/dev/null; then
    ...
    warn "请安装 Node.js 18+ 后运行: cd dashboard && npm install && npm run build"
    return
  fi
  ...
  if [ -f "$REPO_DIR/dashboard/package.json" ]; then
    ...
    if [ -f "$REPO_DIR/dashboard/dist/index.html" ]; then
      log "前端构建Done: dashboard/dist/"
    else
      warn "前端构建可能失败，请手动检查"
    fi
  else
    warn "未找到 dashboard/package.json，跳过前端构建"
  fi
}
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  🎉  U.S. Cabinet Branch安装Done！                          ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
echo ""
echo "下一步："

...
export default function ModelConfig() {
  ...
  if (!agentConfig?.agents) {
    return <div className="empty" style={{ gridColumn: '1/-1' }}>⚠️ Please start local server first</div>;
  }
  ...
  const applyModel = async (agentId: string) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        toast(agentId + ' model changed', 'ok');
    ...
    <div>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div className="cl-list">
          {!changeLog?.length ? (
            <div style={{ fontSize: 12, color: 'var(--muted)', padding: '8px 0' }}>No changes</div>
          ) : (
            [...changeLog]
              .reverse()
              .slice(0, 15)

...
const CAT_META: Record<string, { icon: string; color: string; desc: string }> = {
  'Politics': { icon: '🏛️', color: '#6a9eff', desc: 'Global politics' },
  'Military': { icon: '⚔️', color: '#ff5270', desc: 'Military & conflicts' },
  'Economy': { icon: '💹', color: '#2ecc8a', desc: 'Economy & markets' },
  'AI': { icon: '🤖', color: '#a07aff', desc: 'AI & LLM progress' },
};

const DEFAULT_CATS = ['Politics', 'Military', 'Economy', 'AI'];
...
export default function MorningPanel() {
  ...
  const refreshNews = async () => {
    ...
    try {
      ...
      toast('Fetch triggered, checking for updates…', 'ok');
      let count = 0;
      if (pollRef.current) clearInterval(pollRef.current);
      pollRef.current = setInterval(async () => {
        ...
        if (count > 24) {
          ...
          setRefreshing(false);
          setRefreshLabel('⟳ Refresh Now');
          toast('Fetch timeout, please retry', 'err');
          return;
        }
        try {
  ...
  const addFeed = (name: string, url: string, category: string) => {
    if (!localConfig || !name || !url) {
      toast('Please fill in source name and URL', 'err');
      return;
    }
  ...
  return (
    <div>
      ...
      {!Object.keys(cats).length ? (
        <div className="mb-empty">No data, click "Refresh Now" to fetch today's brief</div>
      ) : (
  ...
  return (
    <div className="sub-config" style={{ marginBottom: 20, padding: 16, background: 'var(--panel2)', borderRadius: 12, border: '1px solid var(--line)' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Categories</div>
      ...
      {/* Keywords */}
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Keywords</div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          <input placeholder="Source name" value={feedName} onChange={(e) => setFeedName(e.target.value)}

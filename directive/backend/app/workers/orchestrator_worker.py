...
log = logging.getLogger("directive.orchestrator")
...
class OrchestratorWorker:
    ...
    async def _on_task_created(self, payload: dict, trace_id: str):
        ...
        agent = STATE_AGENT_MAP.get(TaskState(state), "chief_of_staff")
    ...
    async def _on_task_stalled(self, payload: dict, trace_id: str):
        """任务停滞 → 通知 Senate / Supreme Court 协调。"""
        task_id = payload.get("task_id")
